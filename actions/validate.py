...
class ValidateLearnForm(ValidationAction):
    ...
    def validate_subject_learn(
        ...
        try:
            ...
            for subject in subjects:
                ...
                subjectsLower.append(subject)
            print("Validate subjects list: {}".format(subjects))
            if slot_value.lower() in subjectsLower:
                print("validate_subject: {}".format(slot_value))
                position = subjectsLower.index(slot_value.lower())
                return {"subject_learn": subjects[position]}
            else:
                print("validate_subject else: {}".format(slot_value))
                dispatcher.utter_message(
                    text="I'm sorry, I didn't understand the subject.")
    ...
    def validate_lesson_learn(
        ...
        if(int(slot_value) <= api.getLessons(subject)):
            ...
            dispatcher.utter_message(text="There are only {} lessons in {}".format(
                api.getLessons(subject), subject))
    ...
    def validate_topic_learn(
        ...
        if str(slot_value).lower() in topics:
            ...
            for t in api.getTopics(subject, grade, lesson):
                topics = topics + " " + t
            dispatcher.utter_message(
                text="I did not get that. Please try again.".format(topics))
